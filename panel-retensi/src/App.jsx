...
const NEG_WORDS = [
  "rusak","kecewa","jelek","tidak sesuai","lambat","buruk","cacat",
  "mengecewakan","parah","jangan beli","busuk","penyok","bocor","sobek",
  "patah","menyesal","zonk","tipu","palsu","bau","kotor","hancur","lecet","retak",
];
const NEGATIONS = ["tidak","bukan","ga","gak","nggak","belum","tanpa","anti"];
...
function detectSentiment(text) {
  ...
  NEG_WORDS.forEach((w) => {
    ...
    if (idx === -1) return;
    const before = lower.slice(Math.max(0, idx - 28), idx).trim().split(/\s+/);
    const negated = before.slice(-3).some((b) => NEGATIONS.includes(b));
    if (!negated) flagged.push(w);
  });
  return { isNeg: flagged.length > 0, score: Math.min(1, flagged.length / 3), words: flagged };
}
...
export default function App() {
  ...
  const [orders, setOrders]     = useState(() => ORDERS_INIT.map(o => ({...o, spLevel:calcSpLevel(o.elapsed)})));
  const [customers, setCustomers] = useState(CUSTOMERS_RAW);
  const [toasts, setToasts]     = useState([]);
  const [logEntries, setLogEntries] = useState([]);
  const [clock, setClock]       = useState(new Date());
  const [simSpeed, setSimSpeed] = useState(1); // 1 = normal, >1 = skip
  ...
  useEffect(() => {
    const TICK_MS   = 1000;
    const SIM_H_PER_TICK = (2 / 60) * simSpeed; // 2 min/sec * speed
      ...
      setOrders(prev => prev.map(o => {
        ...
        if (newLevel > o.spLevel) {
          for (let lvl = o.spLevel + 1; lvl <= newLevel; lvl++) {
            ...
            if (lvl === 1) {
              newLogs.push({id, time:nowStr, col:SP_COLORS[1], text:`SP1 otomatis → ${o.seller} (${o.code}) melewati ${SP1_H} jam tanpa pengiriman.`});
              newToasts.push({id:"t"+id, title:"SP1 terkirim", tc:SP_COLORS[1], body:`${o.code} · ${o.seller}`});
            } else if (lvl === 2) {
              newLogs.push({id, time:nowStr, col:SP_COLORS[2], text:`SP2 eskalasi → ${o.seller} (${o.code}) melewati ${SP2_H} jam.`});
              newToasts.push({id:"t"+id, title:"Eskalasi SP2", tc:SP_COLORS[2], body:`${o.code} · ${o.seller}`});
            } else {
              newLogs.push({id, time:nowStr, col:SP_COLORS[3], text:`SP3: ${o.seller} diblokir — refund ${o.code} diproses otomatis (>${BLOCK_H} jam).`});
  ...
  const sendVoucher = useCallback((custId) => {
    ...
    const id = ++logSeq.current;
    setLogEntries(p => [{id, time:nowStr, col:"oklch(0.72 0.08 178)", text:`Voucher win-back 15% terkirim ke ${cust?.name} (${custId}) — segmen At-Risk.`}, ...p].slice(0,50));
  }, [customers]);
  ...
  const submitReview = useCallback(() => {
    ...
    setTimeout(() => {
      const {isNeg, score, words} = detectSentiment(reviewText);
      // MISMATCH hanya jika rating >= 4 DAN sentimen negatif
      const isMismatch = rating >= 4 && isNeg;
      setAnalyzing(false);
      const confLabel = score === 0 ? "—" : score < 0.34 ? "rendah" : score < 0.67 ? "sedang" : "tinggi";

      if (isMismatch) {
        setResultBanner({type:"mismatch", words, text:reviewText, icon:"⚠",
          msg:`Rating ${rating}★ tidak konsisten dengan sentimen teks. Kata kunci: ${words.join(", ")}.`,
          conf:confLabel, bg:"oklch(0.24 0.05 40)", border:"oklch(0.68 0.15 40/0.5)", fg:"oklch(0.85 0.09 55)"});
        setShake(true); setTimeout(() => setShake(false), 500);
        setFlagHistory(p => [{id:Date.now(), time:new Date().toLocaleTimeString("id-ID",{hour12:false}), rating, text:reviewText, words}, ...p].slice(0,10));
      } else if (isNeg && rating < 4) {
        // konsisten negatif
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:18, paddingBottom:0, flexWrap:"wrap", gap:12}}>
          ...
          <div style={{display:"flex", alignItems:"center", gap:14}}>
            <div style={{display:"flex", alignItems:"center", gap:8}}>
              <span style={{font:`500 10px ${mono}`, color:dim, textTransform:"uppercase", letterSpacing:".08em"}}>Simulasi</span>
              ...
              <button onClick={() => setOrders(p => p.map(o => {
                const el = Math.min(o.elapsed + 24, BLOCK_H + 5);
                return {...o, elapsed:el, spLevel:calcSpLevel(el)};
              }))} style={{background:"oklch(0.8 0.11 68/0.15)", border:"1px solid oklch(0.8 0.11 68/0.4)", color:gold, font:`500 11px ${mono}`, padding:"5px 11px", borderRadius:8, cursor:"pointer"}}>
                ⏭ Skip +24j
              </button>
            </div>
                ...
                {resultBanner && (
                  <div style={{marginTop:12, background:resultBanner.bg, border:`1px solid ${resultBanner.border}`, color:resultBanner.fg, padding:"13px 15px", borderRadius:12, animation:"popIn .3s ease"}}>
                    ...
                    <div style={{marginTop:8, font:`500 10px ${mono}`, color:"oklch(0.55 0.01 60)", letterSpacing:".04em"}}>
                      confidence sinyal negatif: <span style={{color:resultBanner.fg}}>{resultBanner.conf}</span>
                    </div>
                  </div>
                )}
              </div>
